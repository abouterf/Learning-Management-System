...

{
    ...
    public function test_authenticated_user_can_add_categories()
    {
        $this->actingAsAdmin();
        $this->createCategory();
    ...
    public function test_authenticated_user_can_update_categories()
    {
        ...
        $this->createCategory();
        $this->assertEquals(1, Category::all()->count());
        $this->patch(route('categories.update', 1), ['title' => $newTitle,
    ...
    {
        $this->post(route('categories.store'), ['title' => $this->faker->word,
            'slug' => $this->faker->word]);
    }
}
